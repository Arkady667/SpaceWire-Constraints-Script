...
def excel_pulse():
    ...
    pulse.write_merge(3, 3, 1, 9, "BIT_PERIOD - D_SKEW - Longest[Data to FF:CLK(Rise/Fall)] + Shortest[Strobe to FF:CLK(Rise/Fall)] - Setup FF - Hold FF ", style=styleFormulaRight)
    pulse.write(4, 1, "If Data Skew", style=styleText)
    pulse.write_merge(5, 5, 1, 9, "BIT_PERIOD - S_SKEW - Longest[Strobe to FF:CLK(Rise/Fall)] + Shortest[Data to FF:CLK(Rise/Fall)] - Setup FF - Hold FF ", style=styleFormulaRight)
...
def main(tcl_script_path, stb_in_name, dat_in_name, reg_filter_rise,reg_filter_fall, error_flag):
    ...
    print type(setup_check(rise, rise))
    excel()
    # print spw_timing_report_list
